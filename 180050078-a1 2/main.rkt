...
(define (crack-cipher strategies key) ;; Returns list of encryption keys.
  ...
  (define s1 ((car strategies) key ))
  (define s2 ((cadr strategies) key))
  (define s3 ((caddr strategies) key))
  (let* ([res1  (work s1 '() key 0 #f)]
         [res (remove-duplicates (append (caar res1)(caar (work s2 '() key (cdr res1) #f))))])
    (if (null? res) (caar (work s3 '() key (cdr res) #f))
        res)))
    ;(if(not (cdar res)) (remove-duplicates (append (caar res)(caar (work s2 '() key (cdr res) #f))))
       ;(caar res))))

(define (work s ans key id bool)
  (if (null? s) (cons (cons ans #f) id)
     (let*  ([key1 (utils:add-substitution (car s) key)]
            [key2 (algo:dictionary-closure key1)])
     (if (not key2) (work (cdr s) ans key id bool)
         (let ([key3 (algo:secret-word-enumeration key2)])
          ; (displayln key3)
           (if key3 (begin (write-char #\@)(write-char #\@)(write-char #\@)(write-char #\+)(write-char #\space)(display id)
                           (write-char #\newline)(utils:show-key key3) 
                           (write-char #\@)(write-char #\@)(write-char #\@)(write-char #\-)(newline)
                           (let ([tmp (algo:secret2 (algo:secret key3) (+ id 1) '())])
                           (work (cdr s) (append (car tmp) ans) key (cdr tmp) (is-com? key3))))
              (work (cdr s) ans key id bool)))))))

(define (is-com? k)
  (andmap (lambda (x) (if(equal? #\_ x) #f #t)) k))
         
     
(define empty (make-list 26 #\_))
;(member (utils:encryption-key "strnin") (crack-cipher strat:composition empty) ) 
;(crack-cipher strat:composition empty)    
